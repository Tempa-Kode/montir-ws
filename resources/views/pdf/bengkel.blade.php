...
<body>
    <!-- Header Section -->
    <div class="header">
        <h1>LAPORAN DATA BENGKEL</h1>
        <div class="subtitle">Montir App</div>
        <div class="subtitle">Data Bengkel Terverifikasi</div>
    </div>
    ...
    <div class="report-info">
        <div class="left">
            ...
            <div class="value">{{ \Carbon\Carbon::now()->format('d F Y, H:i') }} WIB</div>
        </div>
        <div class="right" style="text-align: right;">
            <div class="label">Total Bengkel:</div>
            <div class="value">{{ count($data ?? []) }} Bengkel Terverifikasi</div>
        </div>
    </div>

    <!-- Section Title -->
    <div class="section-title">Data Bengkel Terverifikasi</div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 25%;">Nama Bengkel</th>
            ...
            <tbody>
                @foreach($data as $index => $bengkel)
                    <tr>
                        <td style="text-align: center;">
                            <span class="row-number">{{ $index + 1 }}</span>
                        </td>
                        <td>
                            <div class="bengkel-name">{{ $bengkel->user->nama ?? $bengkel->nama ?? 'N/A' }}</div>
                            <div class="contact-info">Pemilik: {{ $bengkel->user->nama }}</div>
                        </td>
                        <td class="address">
                            {{ $bengkel->user->alamat ?? $bengkel->alamat ?? 'Alamat tidak tersedia' }}
                        </td>
                        <td class="contact-details">
                            <div><strong>Telp:</strong> {{ $bengkel->user->no_telp ?? 'N/A' }}</div>
                            <div><strong>Email:</strong> {{ $bengkel->user->email ?? 'N/A' }}</div>
                        </td>
                        <td>
                            ...
                            <div class="date-time">
                                {{ \Carbon\Carbon::parse($bengkel->created_at)->format('H:i') }} WIB
                            </div>
                        </td>
                    </tr>
                @endforeach
